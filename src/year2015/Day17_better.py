...
class Day17(Day):
    ...
    def makeLookup(self, data):
        amounts = {i: sum([x==i for x in data] for i in data)}
        # lookup = {1: sum([x==1 for x in data])}
        # reverse = list(reversed(unique))
        for i in amounts:
            amount = amounts[i]
            for times in range(2, amount):
                amounts[i*times] = amount()
            for j in amounts:
                if j <= i:
                    continue
...
Day17(17).getResult(testOnly=True)
