...
def updateVisibleGrid(integer_grid, visible_grid):
...
def getAmountOfVisibleTrees(visible_grid):
    amount = 0
...
def getScenicScore(integer_grid, x, y):
...
def generateScenicScores(integer_grid):
    scenic_scores = []
    for y in range(0, len(integer_grid)):
        ...
        for x in range(0, len(integer_grid)):
            if 0 < y < len(integer_grid)-1 and 0 < x < len(integer_grid)-1:
                scenic_scores[len(scenic_scores)-1].append(getScenicScore(integer_grid, x, y))
            else:
                scenic_scores[len(scenic_scores)-1].append(0)
    return scenic_scores


def findLargestScenicScore(score_grid):
    high_score = 0
...
def solvePartOne(data):
    integer_grid = parseGrid(data)
    visible_grid = setupVisibleGrid(len(integer_grid[0]), len(integer_grid))
    updateVisibleGrid(integer_grid, visible_grid)
    return getAmountOfVisibleTrees(visible_grid)


def solvePartTwo(data):
    integer_grid = parseGrid(data)
    scenic_grid = generateScenicScores(integer_grid)
    return findLargestScenicScore(scenic_grid)
